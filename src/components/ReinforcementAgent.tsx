...
// Static method for reinforcement learning updates
ReinforcementAgent.updateState = (
  ...
  if (isCorrect) {
    ...
    // Positive reinforcement - slight mastery increase
    if (newState.streak > 3) {
      newState.masteryLevel = Math.min(newState.masteryLevel + 0.1, 10);
    }
  } else {
    newState.streak = 0;
    
    // Negative reinforcement - slight mastery decrease
    newState.masteryLevel = Math.max(newState.masteryLevel - 0.05, 1);
  }

  // Simulate topic analysis (simplified)
  const topics = ['algebra', 'geometry', 'calculus', 'statistics'];
  const randomTopic = topics[Math.floor(Math.random() * topics.length)];
  
  if (isCorrect) {
    if (!newState.strongTopics.includes(randomTopic)) {
      newState.strongTopics.push(randomTopic);
    }
    newState.weakTopics = newState.weakTopics.filter(t => t !== randomTopic);
  } else {
    if (!newState.weakTopics.includes(randomTopic)) {
      newState.weakTopics.push(randomTopic);
    }
    newState.strongTopics = newState.strongTopics.filter(t => t !== randomTopic);
  }
